...
//   console.log("all jobs are ...",jobs);
  return <div className="my-16">
    <h1 className="fonts text-center text-[48px] font-bold font-serif">Current Jobs</h1> 
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-3 gap-5">
{
    jobs?.map(job=>{
        ...
        <header className="flex justify-between mb-12">
            <img className="h-[100px] w-[140px]  rounded-[10px] shadow-xl" src={img} alt="" />
            <aside>
            ...
            <div className="">
                ...
                <p className="text-slate-600 text-sm font-medium my-2">{details} And it is dolor sit, amet consectetur adipisicing elit. Veritatis, commodi.</p>
                <p className="mt-5 flex gap-2 text-slate-700">
                    {
                        skills?.map(skill =><span className="shadow-lg py-3 hover:text-[#00ae87] px-2 bg-slate-200 ">{skill?.name} </span> )
                    }
            ...
            <footer className="mt-10 flex justify-between">
                ...
                <label  htmlFor="my-modal-6" className="bg-slate-200 apply fonts p-2 rounded-md text-slate-700 font-medium flex items-center gap-3">Quick Apply <RxTrackNext/> 
                  </label>
              <DetailModal job={job}/>
            </footer>
